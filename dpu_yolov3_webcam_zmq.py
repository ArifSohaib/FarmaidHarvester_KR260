...
cap = cv2.VideoCapture("extractedMeetVideo.mp4")
...
try:
    while True:
        ...
        # Receive a message before processing the frame
        message = socket.recv()

        boxes, scores, classes = run_frame(frame)

        # Serialize the boxes array using pickle
        serialized_boxes = pickle.dumps(boxes)
        socket.send(serialized_boxes)
    ...
    print(f"A ZMQ error occurred: {e}")
